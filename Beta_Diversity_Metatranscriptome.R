#Beta diversity Metatranscriptome
#Beta Diversity
library(tidyverse)
library(pheatmap)
beta_data_metatranscriptome <- read_csv("SI072_NirK_beta_diversity_metatranscriptome.csv")
head(beta_data_metatranscriptome)

beta_data_metatranscriptome <- beta_data_metatranscriptome %>%
  mutate(sample_2 = recode(sample_2, "NirK_complete_profile" = "NirK_10m_complete_profile"))

# Split 'sample_1' and 'sample_2' into their components (optional but useful for clarity)
beta_data_metatranscriptome <- beta_data_metatranscriptome %>%
  mutate(
    sample_1 = gsub("_complete_profile", "", sample_1),
    sample_2 = gsub("_complete_profile", "", sample_2)
  ) %>%
  mutate(
    sample_1 = gsub("NirK_", "", sample_1),
    sample_2 = gsub("NirK_", "", sample_2)
  )%>%
  mutate(
...
all_samples <- sort(unique(c(beta_data_metatranscriptome$sample_1, beta_data_metatranscriptome$sample_2)))
...
for (i in seq_len(nrow(beta_data_metatranscriptome))) {
  row_name <- as.character(beta_data_metatranscriptome$sample_1[i])
  col_name <- as.character(beta_data_metatranscriptome$sample_2[i])
  value <- beta_data_metatranscriptome$Z_1[i]
...
pheatmap(as.matrix(sorted_matrix),
         ...
         border_color = NA,
         main = "KR Distance Heatmap with Dendrogram - Metranscriptome")
